 ...
 * context is appended separately per-request in index.ts.
 */
...
4. One dataset does link ethnicity to a health outcome: deaths_ethnicity_state.json (deaths by state/sex/ethnicity). It has absolute counts only, never a rate — this project has no state-level population-by-ethnicity dataset to normalise against, so never imply one ethnic group has a "higher rate" than another from this data; only describe raw counts, and say explicitly that a per-capita comparison isn't possible here. Always exclude the "overall" ethnicity row from any group-to-group comparison — it's DOSM's own cross-check total, not a real ethnicity. Outside of this one dataset, don't combine or link ethnicity with health-outcome data — ethnicity figures elsewhere (in population data) are population-structure only.
